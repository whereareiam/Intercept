...
class MessageFileRepositoryIntegrationTest extends BaseTest {
	...
	void testInsertAndFindById(DatabaseType type) {
		...
		entity.setNamespace(Constants.Namespace.INTERNAL);
	...
	void testFindByFilePath(DatabaseType type) {
		...
		MessageFileEntity found = repo.findByFilePathAndNamespace(
				Constants.Namespace.INTERNAL, "common/colors").orElse(null);
		assertNotNull(found);
	...
	void testExistsByFilePath(DatabaseType type) {
		...
		assertTrue(repo.existsByFilePathAndNamespace(Constants.Namespace.INTERNAL, "test"));
		assertFalse(repo.existsByFilePathAndNamespace(Constants.Namespace.INTERNAL, "nonexistent"));
	}
	...
	void testCount(DatabaseType type) {
		...
		entity1.setNamespace(Constants.Namespace.INTERNAL);
		entity1.setFilePath("test1");
		repo.save(entity1);

		MessageFileEntity entity2 = new MessageFileEntity();
		entity2.setNamespace(Constants.Namespace.INTERNAL);
