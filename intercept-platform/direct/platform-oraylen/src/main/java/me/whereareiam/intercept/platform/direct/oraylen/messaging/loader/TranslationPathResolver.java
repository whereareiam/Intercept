...
public final class TranslationPathResolver {
	private final String extension;
	private final ParsingUtil utilities;

	TranslationPathResolver(Format format, ParsingUtil utilities) {
	...
	Path resolveGlobRoot(Path baseDirectory, String pattern) {
	...
	boolean looksLikeFilePath(String rawPath, Locale defaultLocale) {
		...
		if (utilities.parseLocaleToken(withoutExt, defaultLocale) != null) return true;

		return fileName.contains(".");
	}

	Path resolveExistingPath(Path path) {
