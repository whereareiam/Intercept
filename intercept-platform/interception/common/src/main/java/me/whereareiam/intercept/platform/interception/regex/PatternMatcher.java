...
class PatternMatcher {
	...
	Optional<String> tryMatchPatterns(String text, Locale locale, List<PatternIndexBuilder.PatternMatch> patternIndex) {
		...
		boolean useLiteralPrefix = regexSettings != null && regexSettings.isUseLiteralPrefix();
		int warnSlowMs = regexSettings != null ? regexSettings.getWarnSlowPatternsMs() : 0;
		...
		for (PatternIndexBuilder.PatternMatch candidate : patternIndex) {
			...
			if (warnSlowMs > 0 && elapsedMs > warnSlowMs) {
