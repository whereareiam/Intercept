...
import me.whereareiam.intercept.event.base.*;
...
public class EventController implements EventManager {
	...
	public void register(EventListener listener) {
		for (Method method : listener.getClass().getDeclaredMethods()) {
			if (method.isAnnotationPresent(IntercepticEvent.class)) {
				Class<?> eventType = method.getParameterTypes()[0];
				EventOrder order = method.getAnnotation(IntercepticEvent.class).value();
